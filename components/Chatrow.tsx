  ...
  return (
    <Link href={`/chat/${id}`} className={`chatRow justify-center ${active && 'bg-gray-700/50'}`}>
        ...
        <ChatBubbleLeftIcon className='h-5 w-5 text-white'/>
         <p className='flex-1 hidden md:inline-flex truncate text-white'>{messages?.docs[messages?.docs.length-1]?.data().text || "New Chat"}</p>
        <TrashIcon className='h-5 w-5 text-gray-700 hover:text-white' onClick={removeChat}/>
    </Link>
